...
def _compute_kpi_totals(sel_year: int, sel_month: int) -> dict:
    """
    Recalcula SÓ os 5 totais dos KPIs do topo (emitido/recebido/vencido/
    confirmado/juros) para um mês/ano qualquer — mesma lógica e mesmas fontes
    (asaas_payments_sync + itau_boletos) usadas em get_operational_summary,
    extraída para reuso no cálculo do "vs mês anterior". Não decide nada sobre
    o mês selecionado pelo usuário — é só uma segunda chamada, para o mês-1.
    """
    def _row_to_dict(r):
        return {
            "value":         r.value or 0,
    ...
    try:
        ...
        try:
            rows = _db.execute(text("""
                SELECT value, value_original, net_value, due_date, payment_date, credit_date, status
    ...
    try:
        ...
        recebidos = [p for p in all_payments if p.get("status") in ("RECEIVED", "CONFIRMED")]
...
async def get_operational_summary(
    ...
    # ── Variação dos 5 KPIs do topo vs mês anterior (para os chips ▲/▼) ──
    # Best-effort: se a query do mês anterior falhar por qualquer motivo, os
    # KPIs principais acima já foram calculados e a resposta segue normal,
    # só sem os chips (variacao vem None e o frontend não os exibe).
    _MESES_ABREV = ["", "Jan", "Fev", "Mar", "Abr", "Mai", "Jun",
                    "Jul", "Ago", "Set", "Out", "Nov", "Dez"]
    try:
        _mes_ant = sel_month - 1 if sel_month > 1 else 12
        _ano_ant = sel_year if sel_month > 1 else sel_year - 1
        _prev = _compute_kpi_totals(_ano_ant, _mes_ant)
        ...
        variacao = {
            "mes_anterior_label": f"{_MESES_ABREV[_mes_ant]}/{str(_ano_ant)[2:]}",
            "emitido_pct":    _pct(round(total_emitido, 2),    _prev["emitido"]),
            "recebido_pct":   _pct(round(total_recebido, 2),   _prev["recebido"]),
            "vencido_pct":    _pct(round(total_vencido, 2),    _prev["vencido"]),
            "confirmado_pct": _pct(round(total_confirmado, 2), _prev["confirmado"]),
            "juros_pct":      _pct(round(total_juros, 2),      _prev["juros"]),
        }
    except Exception as _e:
        print(f"⚠️  operational-summary variação vs mês anterior: {_e}")
        variacao = None
